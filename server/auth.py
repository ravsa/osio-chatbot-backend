    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            if not decoded:
                ...
                return dict(error='Authentication failed - token missing'), 401
            ...
            return dict(error='Authentication failed - token has expired'), 401
        except Exception:
            lgr.exception('Failed decoding JWT token')
            decoded = {'email': 'unauthenticated@jwt.failed'}
            return dict(error='Authentication failed - could not decode JWT token'), 401
        else:
        ...
        if user:
            ...
            return dict(error='Authentication failed!'), 401
        return view(*args, **kwargs)
    return wrapper
